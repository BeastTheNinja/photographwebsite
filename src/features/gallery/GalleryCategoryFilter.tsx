...
export default function GalleryCategoryFilter({ categories, selectedCategory, onSelectCategory }: GalleryCategoryFilterProps) {
    return (
        <div className="mb-12 flex flex-wrap justify-center gap-2 sm:mb-16 sm:gap-3">
            {categories.map((category) => (
                <button key={category} type="button" onClick={() => onSelectCategory(category)} className={`rounded-full px-5 py-2.5 text-sm transition-all sm:px-8 sm:py-3 ${selectedCategory === category
