...
function GalleryCard({ image, onOpen }: GalleryCardProps) {
    return (
        <button type="button" className="relative block w-full overflow-hidden rounded-2xl shadow-lg cursor-pointer group text-left" onClick={() => onOpen(image)}>
            <GalleryImage src={image.src} alt={image.alt} className="h-56 w-full object-cover transition-transform duration-500 group-hover:scale-110 sm:h-64 md:h-72" placeholderLabel={image.category} />
