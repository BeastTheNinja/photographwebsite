...
export default function GalleryImage({ src, alt, className, placeholderLabel = 'Billede kommer snart', priority = false }: GalleryImageProps) {
    ...
    return (
        <div className="relative overflow-hidden">
            ...
            <Image src={src} alt={alt} width={1200} height={900} className={`${className} transition-all duration-300 ${isLoaded ? 'blur-0 scale-100 opacity-100' : 'blur-md scale-[1.03] opacity-70'}`} onError={() => setFailed(true)} onLoad={() => setIsLoaded(true)} priority={priority} placeholder="blur" blurDataURL={BLUR_DATA_URL} />
        </div>
    );
}